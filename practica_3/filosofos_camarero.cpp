...
void funcion_camarero() {
   int sitios_libres = num_filosofos -1;
   int valor, tag_valida;
   MPI_Status estado;

   while(true) {
      if(sitios_libres == 0)
         tag_valida = etiq_levantarse;
      else
         tag_valida = MPI_ANY_TAG;
      
      MPI_Recv(&valor, 1, MPI_INT, MPI_ANY_SOURCE, tag_valida, MPI_COMM_WORLD, &estado);
