...
void funcion_consumidor(int orden)
{
   ...
   for (unsigned int i = 0; i < num_items / numero_consumidores; i++)
   {
      ...
      consumir(valor_rec);
      buff[0] = valor_rec;
      MPI_Send(&buff, 2, MPI_INT, id_registro, etiq_consumidor, MPI_COMM_WORLD);
   }
}
// ---------------------------------------------------------------------
...
void funcion_buffer()
{
   ...
   for (unsigned int i = 0; i < num_items * 2; i++)
   {
      ...
      case etiq_productor: // si ha sido el productor: insertar en buffer
         ...
         MPI_Send(&buff_registro, 2, MPI_INT, id_registro, etiq_buff_insertar, MPI_COMM_WORLD);
      ...
      case etiq_consumidor: // si ha sido el consumidor: extraer y enviarle
         ...
         buff_registro[0] = valor;
         MPI_Send(&buff_registro, 2, MPI_INT, id_registro, etiq_buff_extraer, MPI_COMM_WORLD);
...
void funcion_registro()
{
   ...
   // mensajes de todos los productores
   for (int i = 0; i < num_items; i++)
   {
      MPI_Recv(&recv, 2, MPI_INT, MPI_ANY_SOURCE, etiq_productor, MPI_COMM_WORLD, &estado);
      cout << "El valor " << recv[0] << " ha sido producido por el productor " << recv[1] << endl;
   }

   // mensajes de todos los consumidores
   for (int i = 0; i < num_items; i++)
   {
      MPI_Recv(&recv, 2, MPI_INT, MPI_ANY_SOURCE, etiq_consumidor, MPI_COMM_WORLD, &estado);
   ...
   {
      MPI_Recv(&recv, 2, MPI_INT, id_buffer, MPI_ANY_TAG, MPI_COMM_WORLD, &estado);
